...
class PI0Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        print("Max allocated:", round(torch.cuda.max_memory_allocated(0)/1024**2, 2), "MB")
        print("==============================\n")
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            # if "model.paligemma_with_expert.paligemma.model.language_model.embed_tokens.weight" in missing_keys:
            #     try:
            #         print("⚙️ Detected missing embed_tokens.weight — tying weights with outer lm_head.weight")

            #         with torch.no_grad():
            #             # 正确路径：embed 在 language_model 里，lm_head 在 paligemma 外层
            ...
            #             print("✅ Weight tying complete. Shared memory:", same_tensor)

            #         # 移除 missing key 记录
            #         missing_keys.remove("model.paligemma_with_expert.paligemma.model.language_model.embed_tokens.weight")

            #     except Exception as e:
            #         print(f"❌ Failed to tie weights automatically: {e}")
            if missing_keys:
